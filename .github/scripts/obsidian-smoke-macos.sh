...
plugin_path="${vault_path}/.obsidian/plugins/knowledge-base-agent"
...
cleanup() {
  ...
  if [[ "${exit_status}" -ne 0 && -x "${cli_path}" && -S "${cli_socket}" ]]; then
    ...
    "${cli_path}" 'vault=smart-composer-ci' plugin id=knowledge-base-agent \
      > "${artifact_dir}/failure-plugin.txt" 2>&1 || true
    "${cli_path}" 'vault=smart-composer-ci' eval \
      'code=JSON.stringify({layoutReady: app.workspace?.layoutReady === true, pluginLoaded: Boolean(app.plugins.getPlugin("knowledge-base-agent")), settingsPopoutConfigured: app.vault?.getConfig("settingsPopoutWindow") === true, settingsPopoutOpen: app.setting?.isPopoutModal?.() === true, settingsDocumentIsMain: app.setting?.modalEl?.ownerDocument === document, settingsTabRegistered: app.setting?.pluginTabs?.some(tab => tab.id === "knowledge-base-agent") === true, settingsRoot: document.querySelector(".smtcmp-settings-root") !== null, loadError: document.querySelector(".smtcmp-settings-load-error") !== null, cardCount: document.querySelectorAll("[data-runtime-provider]").length, modalCount: document.querySelectorAll(".modal").length})' \
...
run_cli plugin:enable id=knowledge-base-agent filter=community
eval_obsidian 'globalThis.__smtcmpPreviousPlugin = app.plugins.getPlugin("knowledge-base-agent"); true' \
  | is_true_output
run_cli plugin:reload id=knowledge-base-agent
wait_for_true \
  'Boolean(app.plugins.getPlugin("knowledge-base-agent")) && app.plugins.getPlugin("knowledge-base-agent") !== globalThis.__smtcmpPreviousPlugin' \
...
wait_for_true \
  'app.setting?.pluginTabs?.some(tab => tab.id === "knowledge-base-agent") === true' \
  'the registered Smart Composer settings tab'
eval_obsidian 'Boolean(app.setting.openTabById("knowledge-base-agent"))' \
  | is_true_output
wait_for_true \
...
eval_obsidian 'JSON.stringify({appVersion: app.getVersion?.() ?? null, pluginLoaded: Boolean(app.plugins.getPlugin("knowledge-base-agent")), settingsOpen: document.querySelector(".modal.mod-settings") !== null})' \
